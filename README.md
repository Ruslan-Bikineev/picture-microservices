...
Проект реализован на **Java 21** с использованием **Spring Boot** и основан на принципах **SOLID**, *
...
- **Redis** используется для кэширования данных в микросервисах User, Image и Comment.
- **Redis Insight** для мониторинга и анализа Redis в рабочем режиме.
- **Prometheus** собирает метрики производительности микросервисов (время отклика, использование памяти, состояние healthcheck). 
- **Grafana** используется для визуализации метрик и централизованного мониторинга состояния системы.
...
База данных для сервисов схематически выглядит так:
...
В проекте настроен централизованный мониторинг микросервисов с использованием Prometheus и Grafana.

- По пути https://picture.ru/grafana доступна Grafana
- Логин: admin
- Пароль: admin
...
- **DELETE http://127.0.0.1:8080/api/v1/users/{user_id}/collections** - Метод удаления изображения из коллекции
  пользователя.
- **POST http://127.0.0.1:8080/api/v1/users/{user_id}/collections** - Метод добавления изображения в коллекцию
  пользователя.
...
- **POST http://127.0.0.1:8080/api/v1/users/registration** - Метод регистрации пользователя (на вход подается json,
  соответствующей структуре, описанной
  ниже).
  ```json
  {
  "username": string,
...
- **Валидация**: Все обязательные поля (username, password) должны быть указаны. В случае ошибки валидации возвращается
  код ошибки 400.
...
- **POST http://127.0.0.1:8080/api/v1/images** - Метод добавления (загрузки) нового изображения (на вход подается json,
...
- **Валидация**: Все обязательные поля (image_base64, user_id,) должны быть указаны. В случае ошибки валидации
  возвращается код ошибки 400.

### Сервис Комментариев

- **DELETE http://127.0.0.1:8080/api/v1/images/{image_id}/comments/{comment_id}** - Метод удаления комментария к
  изображению.
- **GET http://127.0.0.1:8080/api/v1/images/{image_id}/comments/{comment_id}** - Метод получения конкретного комментария
  к изображению.
- **GET http://127.0.0.1:8080/api/v1/images/{image_id}/comments** - Метод получения всех комментариев к изображению.
- **POST http://127.0.0.1:8080/api/v1/images/{image_id}/comments** - Метод добавления комментария к изображению (на вход
  подается json, соответствующей структуре,
  описанной ниже).
  ```json
  {
    "comment_text": string, 
...
- **DELETE https://www.picture.ru/api/images/{image_id}/comments/{comment_id}** - Метод удаления своего комментария к
  изображению.
